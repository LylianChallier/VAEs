...
st.set_page_config(
    page_title="Variational Autoencoder Explorer",
    page_icon="🖼",
    layout="wide"
)
...
def modelisation(dataset, latent_dim, hidden_layers, reconstruction_error, batch_size, epochs):
    ...
    c, w, h = dim
    model = VAE(c, w, h, latent_dim=latent_dim, hidden_dims=hidden_layers, recon_loss_type=reconstruction_error)
    ...
    if os.path.exists(model_path) and os.path.exists(losses_path):
        ...
        with results_container:
            ...
            ax_loss.scatter(range(len(losses['train_losses'])), losses['train_losses'], color='blue')
            ax_loss.plot(losses['test_losses'], 'r-', label='Test Loss')
            ax_loss.scatter(range(len(losses['test_losses'])), losses['test_losses'], color='red')
        ...
        st.success(f"Entrainement terminé après {epochs} epochs! Modèle sauvegardé dans {model_path}")
    ...
    with results_container:
        ...
        visualize_reconstructions(model, test_batch_for_recon, num_images=4, fig=fig_recon)
        st.pyplot(fig_recon)
        plt.close(fig_recon)
...
with col2:
    ...
    vae_diagram= create_vae_diagram(input_dim=dim, latent_dim=latent_dim, hidden_dims=hidden_layers)
...
with col1:
    ...
    # Footer
    st.markdown("---")
    st.markdown("Développée par [Lylian Challier](https://github.com/LylianChallier?tab=repositories) et Mohammed-Amine Grini avec Streamlit et Pytorch.")