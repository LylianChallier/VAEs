...
def create_vae_diagram(input_dim, latent_dim, hidden_dims):
    ...
    graph.node("Reshape", f"Redimensionne\n{flat_size} → {current_channels}×{current_height}×{current_width}")
    ...
    if len(hidden_dims_reversed) == 1:
        ...
        graph.node("Dec0", f"Convolution Transposée\n{current_channels} → {current_channels}\n{current_height}×{current_width} → {next_height}×{next_width}")
        ...
        for i in range(len(hidden_dims_reversed) - 1):
            ...
            node_name = f"Dec{i}"
            graph.node(node_name, f"ConvTranspose2D\n{in_channels} → {out_channels}\n{current_height}×{current_width} → {next_height}×{next_width}")
        ...
        graph.node("Dec_final1", f"ConvTranspose2D\n{current_channels} → {current_channels}\n{current_height}×{current_width} → {next_height}×{next_width}")
        ...
        graph.node("Dec_final2", f"Conv2D\n{current_channels} → {input_channels}\n{current_height}×{current_width} → {current_height}×{current_width}")
        graph.edge("Dec_final1", "Dec_final2")
        
        # Output
        graph.node("Output", f"Output Image\n{input_channels}×{input_height}×{input_width}")
