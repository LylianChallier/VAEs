    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            if status_text is not None and batch_idx % 10 == 0:
                status_text.text(f"Epoch {epoch+1}/{num_epochs} - Batch {batch_idx}/{len(train_loader)}")
        ...
        if images_placeholder is not None and (epoch % 2 == 0 or epoch == num_epochs - 1):
            visualize_reconstructions(model, test_batch, images_placeholder, epoch, recon_fig)
